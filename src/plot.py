        ...
        try:
            ...
            if plotType in self.settingsWindow.univariateDistPlot:  # Univariate distribution
                ...
                if pTest != "None" and plotId != "Pool":
                    annotator = eval("Annotator(self.ax, [(str(i[0]), str(i[1])) for i in [{}]], data=data, y={}, x={}, order=order, verbose=False)".format(pPairs,plotKey, mode))
                    eval("annotator.configure(test=\"{}\", text_format='star', loc='inside')".format(pTest))
                    annotator.apply_and_annotate()
                    pLegend= "p-value annotation legend:\nns: p <= 1.00e+00\n*: 1.00e-02 < p <= 5.00e-02\n**: 1.00e-03 < p <= 1.00e-02\n***: 1.00e-04 < p <= 1.00e-03\n****: p <= 1.00e-04" # No custom threshold allowed
                    detail = pLegend +"\n\n" + "\n".join([" vs. ".join([struct["label"]for struct in i.structs]) + ": " + i.formatted_output for i in annotator.annotations])
